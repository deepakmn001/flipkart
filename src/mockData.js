export const products = [
  {
    id: 1,
    name: "MILTON Euroline Combo Set Go Electro Stainless Steel Electric Kettle, 1.5 Litres, Silver And Flip Lid Thermosteel Hot Or Cold Stainless Steel Water Bottle With Jacket, 1 Litre, Silver, 1500 Watts",
    image: "https://m.media-amazon.com/images/I/71TKAx1ePFL._SL1500_.jpg",
    originalPrice: 2999,
    discountedPrice: 399,
    discount: 87,
    rating: 4.6,
    ratingCount: 8402
  },
  {
    id: 2,
    name: "Bajaj Rex Mixer Grinder 750W 4 Mixer Jars|Mixie for Kitchen with Nutri-Pro Feature Titan Motor-Heavy Duty Grinding Adjustable Speed Control",
    image: "https://rukminim2.flixcart.com/image/416/416/xif0q/mixer-grinder-juicer/h/m/w/-original-imah27qqvfpupktz.jpeg",
    originalPrice: 2000,
    discountedPrice: 399,
    discount: 80,
    rating: 4.6,
    ratingCount: 6361
  },
  {
    id: 3,
    name: "SAMSUNG 80 cm (32 Inch) HD Ready LED Smart Tizen TV with with Bezel-Free Design",
    image: "https://rukminim2.flixcart.com/image/416/416/xif0q/television/g/l/j/-original-imah3868qdatnqg8.jpeg",
    originalPrice: 18990,
    discountedPrice: 497,
    discount: 97,
    rating: 4.6,
    ratingCount: 8336
  },
  {
    id: 4,
    name: "Prestige 5 Litres Svachh Nakshatra Plus Induction Base Inner Lid Hard Anodised Pressure Handi |Black | Deep Lid | Anti-Bulge Base | Sturdy Handles",
    image: "https://m.media-amazon.com/images/I/713Kl1I0CyL._SL1500_.jpg",
    originalPrice: 4999,
    discountedPrice: 299,
    discount: 94,
    rating: 4.9,
    ratingCount: 8314
  },
  {
    id: 5,
    name: "vivo V50 5G (Rose Red, 8GB RAM, 256GB Storage)",
    image: "https://m.media-amazon.com/images/I/71m7OmKyA5L._SL1500_.jpg",
    originalPrice: 36990,
    discountedPrice: 899,
    discount: 98,
    rating: 4.8,
    ratingCount: 7191
  },
  {
    id: 6,
    name: "Panasonic 25L Solo Microwave Oven (NN-ST310QBFG, Black, 85 Auto Cook Menus)",
    image: "https://m.media-amazon.com/images/I/712wBdtn5AL._SL1500_.jpg",
    originalPrice: 8999,
    discountedPrice: 499,
    discount: 94,
    rating: 4.9,
    ratingCount: 7725
  },
  {
    id: 7,
    name: "Philips Viva Collection HD4928/01 2100-Watt Induction Cooktop, Soft Touch Button with Crystal Glass (Black)",
    image: "https://m.media-amazon.com/images/I/715L3WOq4JL._SL1500_.jpg",
    originalPrice: 5939,
    discountedPrice: 449,
    discount: 92,
    rating: 4.8,
    ratingCount: 8661
  },
  {
    id: 8,
    name: "Havells Instanio 3L Instant Water Heater(Geyser)| Temp. Sensing LED Indicator| Rust & Shock Proof Body| ISI Marked Warranty: 5 year on Inner Tank| High Rise Compatible (White Blue)",
    image: "https://m.media-amazon.com/images/I/51+Bh5W2orL._SL1500_.jpg",
    originalPrice: 6999,
    discountedPrice: 499,
    discount: 93,
    rating: 4.7,
    ratingCount: 6405
  },
  {
    id: 9,
    name: "Orient Electric Areva Portable Room Heater | 2000W | Two Heating Modes | Advanced Overheat Protection | Horizontal & Vertical Mount | 1-year replacement warranty by Orient | White",
    image: "https://m.media-amazon.com/images/I/71QfC+NELFL._SL1500_.jpg",
    originalPrice: 4500,
    discountedPrice: 478,
    discount: 89,
    rating: 4.6,
    ratingCount: 9854
  },
  {
    id: 10,
    name: "Green Soul Multi-Purpose Ergonomic Office Chair with Adjustable Armrest & Headrest (Brown)",
    image: "https://m.media-amazon.com/images/I/71B0qOLq9OL._SX679_.jpg",
    originalPrice: 8999,
    discountedPrice: 478,
    discount: 95,
    rating: 4.9,
    ratingCount: 8142
  },
  {
    id: 11,
    name: "TEXUM TEK-HK60 Electric Kettle 2L (1500W) + Premium Water Bottle Combo - Double Layer Stainless Steel Body, Quick Boil Technology, Auto Shut Off, Perfect for Hot Water, Black Tea & Coffee",
    image: "https://m.media-amazon.com/images/I/41-x2RfNFvL.jpg",
    originalPrice: 4500,
    discountedPrice: 379,
    discount: 92,
    rating: 4.7,
    ratingCount: 6717
  },
  {
    id: 12,
    name: "Orient Electric Arc Plus 1.5kw Immersion water heater 100% shock proof body|Heavy copper element With Bucket clip| ISI certified 2 years replacement warranty",
    image: "https://m.media-amazon.com/images/I/61JjdIoAtgL._SL1500_.jpg",
    originalPrice: 999,
    discountedPrice: 295,
    discount: 70,
    rating: 4.5,
    ratingCount: 7149
  },
  {
    id: 13,
    name: "Cloth Fusion Fleece Sherpa Blanket for Winter Double Size 500GSM, Soft Heavy Winter Quilt Razai for Comfort & Warmth (88X90 Inches, Navy & Grey)",
    image: "https://m.media-amazon.com/images/I/61n77eHy-UL._SL1500_.jpg",
    originalPrice: 5939,
    discountedPrice: 395,
    discount: 93,
    rating: 4.7,
    ratingCount: 9501
  },
  {
    id: 14,
    name: "Honeywell Air Purifier for Home & Office, 3-in-1, Pre, H13 HEPA, Activated Carbon, Removes 99.99% Pollutants, Allergens, Pet Danger, Smoke, Dust & Pollens - Air touch V1",
    image: "https://m.media-amazon.com/images/I/71ipnrfS-1L._SL1500_.jpg",
    originalPrice: 9999,
    discountedPrice: 425,
    discount: 96,
    rating: 4.5,
    ratingCount: 9911
  },
  {
    id: 15,
    name: "OnePlus Bullets Z2 Bluetooth Wireless in Ear Earphones with Mic, Bombastic Bass, 10 Mins Charge - 20 Hrs Music, 30 Hrs Battery Life (Acoustic Red)",
    image: "https://m.media-amazon.com/images/I/5171kgKguGL._SL1500_.jpg",
    originalPrice: 1799,
    discountedPrice: 299,
    discount: 83,
    rating: 4.7,
    ratingCount: 9391
  },
  {
    id: 16,
    name: "Havells Dry Iron Dazzle 1100 Watts, Iron Press, Greblon Non Sick Soleplate with German Technology, Shock Proof Body, Aerodynamic Design, 360° Swivel Cord, 2 Year Manufacturer Warranty (Black)",
    image: "https://rukminim2.flixcart.com/image/416/416/kbpeQ280/iron/d/p/g/havells-dazzle-havells-original-imagnx2dhpy6e6ub.jpeg",
    originalPrice: 1999,
    discountedPrice: 289,
    discount: 86,
    rating: 4.9,
    ratingCount: 6823
  },
  {
    id: 17,
    name: "KENT Supreme RO Water Purifier | .0001 Micron RO Membran | RO + UF + TDS Control + UV LED Tank | 8L | 20 LPH | Ideal for Borewell/Tanker/Municipal Water | Largest Service Network",
    image: "https://m.media-amazon.com/images/I/51ICtMHR0rL._SL1000_.jpg",
    originalPrice: 1999,
    discountedPrice: 439,
    discount: 78,
    rating: 4.8,
    ratingCount: 9429
  },
  {
    id: 18,
    name: "atomberg Renesa Enzel 1200mm BLDC Ceiling Fan with Remote Control | BEE 5 star Rated Energy Efficient Ceiling Fan | High Air Delivery with LED Indicators | 1+1 Year Warranty (Gloss White)",
    image: "https://m.media-amazon.com/images/I/51GZwXVUYbL._SL1500_.jpg",
    originalPrice: 2699,
    discountedPrice: 419,
    discount: 84,
    rating: 4.7,
    ratingCount: 7020
  },
  {
    id: 19,
    name: "JBL Flip 6 Wireless Portable Bluetooth Speaker Pro Sound, Upto 12 Hours Playtime, IP67 Water & Dustproof, PartyBoost & Personalization App (Without Mic, Black) Visit the JBL Store",
    image: "https://m.media-amazon.com/images/I/71zDU8JBLZL._SL1500_.jpg",
    originalPrice: 8999,
    discountedPrice: 390,
    discount: 96,
    rating: 4.8,
    ratingCount: 7081
  }
];

export const categories = [
  { id: 1, name: "Categories", icon: "grid-3x3" },
  { id: 2, name: "Offer Zone", icon: "percent", badge: "NEW" },
  { id: 3, name: "Mobiles", icon: "smartphone" },
  { id: 4, name: "Fashion", icon: "shirt" },
  { id: 5, name: "Electronics", icon: "laptop" },
  { id: 6, name: "Appliances", icon: "home" }
];